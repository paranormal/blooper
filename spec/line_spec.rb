  ...
  describe Line do
    ...
    before(:each) do
      @logstr = 'Ltime 1352420926.507 response_time 513 src_ip 10.10.0.5' +
        ' squid_request_status TCP_MISS http_status_code 200 reply_size ' +
        '22046 request_method GET request_url' +
        ' http://wiki.squid-cache.org/Features/LogModules username -' +
        ' squid_hier_code HIER_DIRECT dst_ip 77.93.254.178' +
        ' mime_type text/html'
    ...
    it 'should recognise bad encoding' do
      mock_line = @logstr.sub(/513/, '543|Misérаblesё')
      @line.instance_variable_set(:@line, mock_line)
      @line.should be_valid
      mock_line2 = @logstr.sub(/513/,
        'q=��������+�����+������+���������+�������&src=IE-Address')
      @line.instance_variable_set(:@line, mock_line2)
      @line.should be_valid
    end
